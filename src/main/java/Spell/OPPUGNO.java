...
public class OPPUGNO extends SpellProjectile implements Spell {
	public OPPUGNO(Ollivanders plugin, Player player, Spells name, Double rightWand) {
	...
	public void checkEffect() {
		...
		for(LivingEntity e : getLivingEntities(1)) {
			for(SpellProjectile spell : p.getProjectiles()) {
				if(spell instanceof Transfiguration && spell.player.equals(player)) {
					for(Entity entity : player.getWorld().getEntities()) {
						if(entity.getUniqueId() == ((Transfiguration) spell).getToID() && entity instanceof LivingEntity) {
