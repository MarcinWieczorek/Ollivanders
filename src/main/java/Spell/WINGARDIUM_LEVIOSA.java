...
public class WINGARDIUM_LEVIOSA extends SpellProjectile implements Spell {
	...
	public WINGARDIUM_LEVIOSA(Ollivanders plugin, Player player, Spells name, Double rightWand) {
	...
	public void checkEffect() {
		if(moving) {
			...
			if(player.isSneaking()) {
				...
				for(Location loc : locList) {
					Location toLoc = center.clone().add(loc);
					@SuppressWarnings("deprecation") FallingBlock fall = loc.getWorld().spawnFallingBlock(toLoc, materialMap.get(loc), (byte) 0);
