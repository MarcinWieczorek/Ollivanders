...
public class INFORMOUS extends SpellProjectile implements Spell {
	...
	public INFORMOUS(Ollivanders plugin, Player player, Spells name, Double rightWand) {
	...
	public void checkEffect() {
		...
		for(LivingEntity entity : getLivingEntities(1)) {
			if(!iEntity.contains(entity)) {
				player.sendMessage(ChatColor.getByChar(p.getConfig().getString("chatColor")) + entity.getType().toString() + " has " + entity.getHealth() + " health.");
