...
public abstract class Transfiguration extends SpellProjectile {
	...
	public Transfiguration(Ollivanders plugin, Player player, Spells name, Double rightWand) {
			...
			for(Entity e : w.getEntities()) {
				if(e.getUniqueId() == toID) {
					...
					if(fromEType.equals(EntityType.FALLING_BLOCK)) {
						return e.getWorld().spawnFallingBlock(e.getLocation(), fromStack.getType(), (byte) 0);
					}
					else {
